...
const char *SATinstance::badFeatNames[] = {"KLB-featuretime", "lpTIME", "CG-featuretime", "lobjois-featuretime",
                                           "UnitProbeTIME", "POSNEG-RATIO-CLAUSE-max", "UNARY", "VCG-VAR-mean",
                                           "VCG-VAR-coeff-variation", "VCG-CLAUSE-min", "POSNEG-RATIO-CLAUSE-min",
                                           "POSNEG-RATIO-VAR-mean", "POSNEG-RATIO-VAR-min", "POSNEG-RATIO-VAR-entropy",
                                           "HORNY-VAR-mean", "VG-max", "CG-mean", "CG-max", "CG-entropy",
                                           "vars-reduced-depth-256", "gsat_BestStep_Median", "gsat_BestStep_Q.10",
                                           "gsat_BestStep_Q.90", "gsat_FirstLMRatio_CoeffVariance",
                                           "gsat_FirstLMRatio_Mean", "saps_BestStep_Median", "saps_BestStep_Q.10",
                                           "saps_BestStep_Q.90", "gsat_totaltime", "saps_totaltime",
                                           "cluster-coeff-min", "LPSLack-min", "LPSLack-max",
...
SATinstance::SATinstance(const char *filename, bool doComp, long _seed)
    ...
    if (doComp) {
        ...
        fprintf(stderr, "numVars: %d numClauses: %d\n", numVars, numClauses);
        ...
        for (int clauseNum = 0; clauseNum < numClauses; clauseNum++) {
            ...
            while (true) { // skipping comment and empty lines, by Christop P.
                first_char = infile.peek();
                fprintf(stderr, "c first_char: %c\n", first_char); //TODO temp
                if (first_char == 'c' || first_char == '\n') {
                    infile.ignore(1000, '\n'); // ignore this line
                } else {
                    break;
                }
            ...
            infile >> lits[numLits];
//            fprintf(stderr, "lits[%d]: %d\n", numLits,lits[numLits]);
...
// initialize
    ...
    if ((double) numActiveVars == 0) {
        ...
        writeFeature("reducedClauses",
                     ((double) OrigNumClauses - (double) numActiveClauses) / (double) numActiveClauses);
        writeFeature("Pre-featuretime", preTime);
...
// this should not happened
        if (clause_array[t] != 0)
            pos_frac_in_clause[t] = 2.0 * fabs(0.5 - (double) pos_in_clause[t] /
                                                     ((double) pos_in_clause[t] + (double) neg_in_clause[t]));
        else {
            pos_frac_in_clause[t] = RESERVED_VALUE;
...
// == NOTE: double check that indices are correct I.e start with zero
    ...
    for (int i = 0; i < numActiveVars; i++) {
        if (res == OPTIMAL) {
            cons[i] = (solvedInstanceVarPtr[i] < 1.0 - solvedInstanceVarPtr[i] ? solvedInstanceVarPtr[i] : 1.0 -
                                                                                                           solvedInstanceVarPtr[i]);
            if (cons[i] == 0)
                nInt++;
        } else
...
//	printf ("variable %d has depth of %d %d \n", varnr, *_dstackp, *(dstackp-1));
    ...
    while (_dstackp < dstackp) {
        ...
        while (*myclauses != -1) {
            ...
            for (i = 0; i < clauseLengths[_myclause]; i++) {DIAMETER_FIX(abs(clauses[_myclause][i])); }
        }
    }
/*
...
// get all clauses that contain ~lit
        var = ABS(lit);
        for (otherClause = firstClauseWithVar(var, !positive(lit));
             otherClause != NULL; otherClause = nextClauseWithVar()) {
